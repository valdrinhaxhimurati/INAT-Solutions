...
class MateriallagerTab(QWidget):
    ...
    def setup_ui(self):
        ...
        btn_hinzufuegen = QToolButton(); btn_hinzufuegen.setText(_('Material hinzufügen')); btn_hinzufuegen.setProperty("role", "add")
        btn_bearbeiten = QToolButton(); btn_bearbeiten.setText(_('Material bearbeiten')); btn_bearbeiten.setProperty("role", "edit")
        btn_loeschen   = QToolButton(); btn_loeschen.setText(_('Material löschen'));    btn_loeschen.setProperty("role", "delete")
    ...
    def lade_material(self):
        ...
        self.table.setHorizontalHeaderLabels([_("ID"), _("Materialnr."), _("Bezeichnung"), _("Menge"), _("Einheit"), _("Lagerort"), _("Lieferant"), _("Preis"), _("Währung"), _("Bemerkung")])
    ...
    def material_loeschen(self):
        ...
        try:
            with get_db() as con:
                with con.cursor() as cur:
                    try:
                        ...
                        cur.execute(f"DELETE FROM public.materiallager WHERE material_id IN ({placeholders})", tuple(ids))
                    except Exception:
                        placeholders = ','.join(['?'] * len(ids))
                        cur.execute(f"DELETE FROM materiallager WHERE material_id IN ({placeholders})", tuple(ids))
                con.commit()
        except Exception:
            conn = get_db()
            cur = conn.cursor()
            for mid in ids:
                try:
                    cur.execute("DELETE FROM public.materiallager WHERE material_id=%s", (mid,))
                except Exception:
                    cur.execute("DELETE FROM materiallager WHERE material_id=%s", (mid,))
            conn.commit()
            conn.close()
        self.lade_material()
