...
class TerminCard(QFrame):
    ...
    def setup_ui(self):
        ...
        if self.show_weekday:
            ...
            weekday_label.setStyleSheet(f"color: {COLORS['primary']}; font-weight: 600; font-size: {FONT_SIZES['body']}px;")
        ...
        time_label.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: {FONT_SIZES['body']}px;")
        layout.addWidget(time_label)
        
        # Titel
        title_label = QLabel(self.termin_data['titel'])
        title_label.setStyleSheet(f"color: {COLORS['text_primary']}; font-weight: 600; font-size: {FONT_SIZES['h5']}px;")
        ...
        if self.termin_data['kunde_name'] or self.termin_data['ort']:
            ...
            if self.termin_data['kunde_name']:
                ...
                kunde_label.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: {FONT_SIZES['body']}px;")
                info_layout.addWidget(kunde_label)
                
            if self.termin_data['ort']:
                ort_label = QLabel(f"📍 {self.termin_data['ort']}")
                ort_label.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: {FONT_SIZES['body']}px;")
        ...
        if self.termin_data['beschreibung']:
            ...
            desc_label.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: {FONT_SIZES['small']}px;")
...
class AuftragskalenderTab(QWidget):
    ...
    def update_customer_data(self):
        ...
        """
        print(_("[Auftragskalender] Aktualisiere Termine aufgrund von Kundenänderung..."))
        self.load_day_termine()
        self.load_week_termine()
    ...
    def init_ui(self):
        ...
        self.btn_new = QPushButton(_("+ Neuer Termin"))
        ...
        self.btn_new.clicked.connect(self.create_new_termin)
        toolbar_layout.addWidget(self.btn_new)

        for btn_text, btn_name, handler in [
            (_("Bearbeiten"), "btn_edit", self.edit_termin),
            (_("Löschen"), "btn_delete", self.delete_termin),
        ]:
        ...
        calendar_title = QLabel(_("📅 Kalender"))
        calendar_title.setStyleSheet(f"color: {COLORS['text_primary']}; font-size: {FONT_SIZES['h4']}px; font-weight: 600;")
        ...
        self.month_year_label.setStyleSheet(f"color: {COLORS['text_primary']}; font-size: {FONT_SIZES['h5']}px; font-weight: 600;")
        self.month_year_label.setAlignment(Qt.AlignCenter)
        self.month_year_label.setCursor(Qt.PointingHandCursor)
        ...
        day_layout.setContentsMargins(20, 20, 20, 20)
        
        self.day_title = QLabel(_("📌 Termine des Tages"))
        self.day_title.setStyleSheet(f"color: {COLORS['text_primary']}; font-size: {FONT_SIZES['h5']}px; font-weight: 600;")
        ...
        self.day_scroll.setStyleSheet("background: transparent;")
        ...
        # RECHTE SPALTE: Wochenansicht
        week_container = QFrame()
        week_container.setStyleSheet(f"""
            QFrame {{
                background-color: {COLORS['surface']};
                border: 1px solid {COLORS['border']};
                border-radius: 12px;
        ...
        week_layout.setContentsMargins(20, 20, 20, 20)
        
        self.week_title = QLabel(_("📆 Termine der Woche"))
        self.week_title.setStyleSheet(f"color: {COLORS['text_primary']}; font-size: {FONT_SIZES['h4']}px; font-weight: 600;")
        ...
        self.week_scroll.setStyleSheet("background: transparent;")
        ...
        content_layout.addWidget(week_container, 3)
    ...
    def _add_termine_cards(self, layout, termine):
        ...
        if not termine:
            no_data = QLabel(_("Keine Termine vorhanden"))
    ...
    def load_week_termine(self):
        ...
        try:
            ...
            print(_("Fehler beim Laden der Wochentermine: {e}"))
    ...
    def load_day_termine(self):
        ...
        try:
            ...
            print(_("Fehler beim Laden der Tagestermine: {e}"))
    ...
    def highlight_dates_with_appointments(self):
        ...
        try:
            ...
            print(_("Fehler beim Hervorheben: {e}"))
    ...
    def edit_termin(self):
        if not self.selected_card:
            QMessageBox.warning(self, _("Kein Termin"), _("Bitte wählen Sie einen Termin aus."))
            return
    ...
    def delete_termin(self):
        ...
        auftrag_id = self.selected_card.termin_data['id']
        reply = QMessageBox.question(self, _("Termin löschen"), _("Möchten Sie diesen Termin wirklich löschen?"), QMessageBox.Yes | QMessageBox.No)
            ...
            try:
                ...
                if outlook_id and ms_graph and ms_graph.is_connected():
                    try:
                        ...
                        QMessageBox.information(self, _("Erfolg"), _("Termin lokal und in Outlook gelöscht."))
                    except Exception as e:
                        QMessageBox.warning(self, _("Outlook Fehler"), _("Termin lokal gelöscht, aber Fehler in Outlook:\n") + f"{e}")
                else:
                    QMessageBox.information(self, _("Erfolg"), _("Termin gelöscht."))
