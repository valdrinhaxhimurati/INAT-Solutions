...
class ArtikellagerTab(QWidget):
    def __init__(self):
        ...
        btn_hinzufuegen = QToolButton(); btn_hinzufuegen.setText(_('Artikel hinzufügen')); btn_hinzufuegen.setProperty("role", "add")
        btn_bearbeiten = QToolButton(); btn_bearbeiten.setText(_('Artikel bearbeiten')); btn_bearbeiten.setProperty("role", "edit")
        btn_loeschen   = QToolButton(); btn_loeschen.setText(_('Artikel löschen'));     btn_loeschen.setProperty("role", "delete")
    ...
    def lade_artikel(self):
        ...
        self.table.setHorizontalHeaderLabels([_("ID"), _("Artikelnummer"), _("Bezeichnung"), _("Bestand"), _("Lagerort"), _("Preis"), _("Währung")])
    ...
    def artikel_loeschen(self):
        ...
        if not ids:
            return
        if QMessageBox.question(self, _("Löschen bestätigen"), _("Soll(en) die ausgewählten {0} Artikel gelöscht werden?").format(len(ids)), QMessageBox.Yes | QMessageBox.No) != QMessageBox.Yes:
            return
        try:
            with get_db() as con:
                with con.cursor() as cur:
                    try:
                        ...
                        cur.execute(f"DELETE FROM public.artikellager WHERE artikel_id IN ({placeholders})", tuple(ids))
                    except Exception:
                        placeholders = ','.join(['?'] * len(ids))
                        cur.execute(f"DELETE FROM artikellager WHERE artikel_id IN ({placeholders})", tuple(ids))
                con.commit()
        except Exception:
            ...
            with get_db() as con:
                with con.cursor() as cur:
                    for aid in ids:
                        try:
                            cur.execute("DELETE FROM public.artikellager WHERE artikel_id=%s", (aid,))
                        except Exception:
                            cur.execute("DELETE FROM artikellager WHERE artikel_id=%s", (aid,))
                con.commit()
        self.lade_artikel()
