...
from PyQt5.QtWidgets import (
    ...
    QTableWidgetItem, QSizePolicy, QAbstractButton, QGroupBox
)
...
class RechnungDialog(BaseDialog):
    ...
    def __init__(self, kunden_liste, kunden_firmen, kunden_adressen, *args, **kwargs):
        ...
        self.setWindowTitle(_("Rechnung"))
        ...
        # === Kopfdaten ===
        kopf_group = QGroupBox(_("Rechnungsdaten"))
        kopf_group.setStyleSheet(GROUPBOX_STYLE)
        grid = QGridLayout(kopf_group)
        row = 0

        # Kopfbereich
        grid.addWidget(QLabel(_("Rechnungs-Nr:")), row, 0)
        self.le_rechnungsnr = QLineEdit()
        grid.addWidget(self.le_rechnungsnr, row, 1, 1, 3)
        row += 1

        grid.addWidget(QLabel(_("Kunde:")), row, 0)
        ...
        grid.addWidget(QLabel(_("Datum:")), row, 0)
        ...
        try:
            ...
            self.de_datum.setCalendarWidget(cal)
        except Exception:
            pass
        grid.addWidget(self.de_datum, row, 1)

        grid.addWidget(QLabel(_("UID:")), row, 2)
        ...
        # MWST
        grid.addWidget(QLabel(_("MWST %:")), row, 0)
        ...
        # Zahlungsbedingungen
        grid.addWidget(QLabel(_("Zahlungsbedingungen:")), row, 0)
        ...
        self.cb_status.addItem(_("Automatisch (aus Fälligkeit)"), "")
        self.cb_status.addItem(_("offen"), "offen")
        self.cb_status.addItem(_("bezahlt"), "bezahlt")
        self.cb_status.addItem(_("überfällig"), "überfällig")
        grid.addWidget(self.cb_status, row, 1, 1, 3)
        row += 1

        # Abschiedsgruss
        grid.addWidget(QLabel(_("Abschiedsgruss:")), row, 0)
        self.text_abschluss = QTextEdit()
        self.text_abschluss.setFixedHeight(70)
        grid.addWidget(self.text_abschluss, row, 1, 1, 3)
        row += 1

        root.addWidget(kopf_group)

        # === Positionen ===
        pos_group = QGroupBox(_("Positionen"))
        pos_group.setStyleSheet(GROUPBOX_STYLE)
        pos_layout = QVBoxLayout(pos_group)
        
        self.tbl_pos = QTableWidget(0, 4)
        self.tbl_pos.setHorizontalHeaderLabels([_("Beschreibung"), _("Menge"), _("Einzelpreis"), _("Total")])
        ...
        pos_layout.addWidget(self.tbl_pos)
        ...
        self.btn_add = QPushButton(_("Position hinzufügen"))
        self.btn_remove = QPushButton(_("Ausgewählte entfernen"))
        self.btn_from_stock = QPushButton(_("Aus Lager..."))
        ...
        pos_layout.addLayout(btn_row_pos)
        root.addWidget(pos_group)

        # === Summen ===
        sum_group = QGroupBox(_("Beträge"))
        sum_group.setStyleSheet(GROUPBOX_STYLE)
        sum_row = QGridLayout(sum_group)
        sum_row.addWidget(QLabel(_("Zwischensumme:")), 0, 2)
        ...
        sum_row.addWidget(QLabel(_("MWST-Betrag:")), 1, 2)
        ...
        sum_row.addWidget(QLabel(_("Gesamtsumme:")), 2, 2)
    ...
    def _add_position(self, beschreibung: str = "", menge: float = 1.0, epreis: float = 0.0, recalc=True):
        ...
        if self.tbl_pos.rowCount() >= 10:
            ...
            QMessageBox.warning(
                self,
                _("Maximale Anzahl erreicht"),
                _("Es können maximal 10 Positionen pro Rechnung hinzugefügt werden.")
            )
            return
    ...
    def _choose_from_stock(self):
        if self.tbl_pos.rowCount() >= 10:
            QMessageBox.warning(self, _("Limit erreicht"), _("Es können maximal 10 Positionen pro Rechnung hinzugefügt werden."))
            return
        if not HAS_LAGER:
            QMessageBox.information(self, _("Nicht verfügbar"), _("Der Lagerdialog ist nicht verfügbar."))
            return
    ...
    def _accept(self):
        if not self.cb_kunde.currentText().strip():
            QMessageBox.warning(self, _("Fehlende Angaben"), _("Bitte einen Kunden auswählen."))
            return
        self.accept()
