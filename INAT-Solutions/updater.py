...
class UpdateManager(QObject):
    def __init__(
        ...
        # Guard to avoid showing multiple update prompt dialogs
    ...
    def _on_manifest_loaded(self, data: dict) -> None:
        ...
        if remote <= current:
            if self._interactive_request:
                QMessageBox.information(self.parent(), _("Keine Updates"), _("Sie verwenden bereits die neueste Version."))
            return
    ...
    def _start_download(self, manifest: UpdateManifest) -> None:
        # Wenn bereits ein Download-Dialog offen ist, verhindere neuen Download
        if self._download_dialog is not None:
            try:
                QMessageBox.information(self.parent(), _("Update"), _("Ein Download läuft bereits."))
            except Exception:
                pass
            return
    ...
    def _cancel_download(self) -> None:
        ...
        # Schließe den Fortschrittsdialog sofort beim Abbrechen
        try:
    ...
    def _on_checksum_ok(self, file_path: Path, manifest: UpdateManifest) -> None:
        # Aufräumen: Behalte nur die letzten X Installer im Updates-Ordner
        try:
            self._prune_updates_dir(keep=3)
        except Exception:
            pass
        self._prompt_install(file_path, manifest)

    def _prune_updates_dir(self, keep: int = 3) -> None:
        """
        Entfernt ältere Installer aus dem Updates-Ordner und behält die neuesten `keep` Dateien.
        Standardmäßig werden nur die letzten 3 Installer behalten.
        """
        try:
            d = updates_dir()
            if not d.exists():
                return
            # Liste aller Dateien (nicht nur .exe, um keine Reste zu lassen)
            files = [p for p in d.iterdir() if p.is_file()]
            # Sortiere nach Änderungszeit, neueste zuerst
            files.sort(key=lambda p: p.stat().st_mtime, reverse=True)
            # Lösche alle Dateien nach den ersten `keep`
    ...
    def _prompt_install(self, installer_path: Path, manifest: UpdateManifest) -> None:
        ...
        box.setWindowTitle(_("Update installieren"))
        box.setText(msg)
        # Add two explicit buttons with clear labels
        install_restart_btn = box.addButton(_("Installieren und neu starten"), QMessageBox.YesRole)
        install_only_btn = box.addButton(_("Nur installieren"), QMessageBox.NoRole)
    ...
    def _launch_installer(self, installer_path: Path, auto_restart: bool = False) -> None:
        ...
        if app is not None:
            ...
            # Zeige nur eine Informationsbox, wenn kein automatischer Neustart gewählt wurde
            if not auto_restart:
                QMessageBox.information(
                    self.parent(),
                    _("Update gestartet"),
                    _("Der Installer wurde gestartet. Bitte starten Sie die Anwendung nach Abschluss manuell neu."),
                )
            app.quit()
    ...
    def _show_error(self, message: str) -> None:
        QMessageBox.critical(self.parent(), _("Update"), message)