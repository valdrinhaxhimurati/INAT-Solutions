...
# Pfad zur lokalen SQLite-DB (anpassen falls nötig)
DB_PATH = Path(r"C:\ProgramData\INAT Solutions\data\datenbank.sqlite")

# Tabellenreihenfolge, passe an falls nötig
_SYNC_TABLES = ["kunden", "rechnungen", "rechnung_positionen", "buchhaltung"]
...
def _sync_local_to_remote_pg(local_db_path: str, remote_dsn: str, tables=None):
    if psycopg2 is None:
        raise RuntimeError("psycopg2 fehlt (pip install psycopg2-binary)")
...
class DBSyncDialog(BaseDialog):
    def __init__(self, parent=None):
        ...
        row_dir = QHBoxLayout()
        row_dir.addWidget(QLabel("Richtung:"))
        self.combo_dir = QComboBox()
        self.combo_dir.addItems(["Download (remote → lokal)", "Upload (lokal → remote)"])
        ...
        if self.remote_from_settings:
            v.addWidget(QLabel("Verwendete Remote-DB (aus Einstellungen):"))
            lbl = QLabel(self.remote_from_settings)
            lbl.setTextInteractionFlags(lbl.textInteractionFlags() | Qt.TextSelectableByMouse)
            v.addWidget(lbl)
            self.le_remote = None
        else:
            v.addWidget(QLabel("Remote-DB-DSN einfügen (z.B. postgresql://user:pass@host:5432/db):"))
            self.le_remote = QLineEdit()
            v.addWidget(self.le_remote)

        # Backup Checkbox (optional)
        self.cb_backup = QCheckBox("Vorher lokales Backup erstellen (empfohlen)")
        self.cb_backup.setChecked(True)
        v.addWidget(self.cb_backup)
    ...
    def _on_start(self):
        ...
        try:
            ...
            if not remote_dsn:
                raise RuntimeError("Kein Remote-DB-DSN angegeben. Füge einen Link ein oder konfiguriere ihn in den Einstellungen.")
            ...
            if self.cb_backup.isChecked():
                try:
                    ...
                    # nicht fatal, nur informieren
                    QMessageBox.warning(self, "Backup fehlgeschlagen", f"Lokales Backup konnte nicht erstellt werden:\n{e}")

            local_db = str(DB_PATH)

            if direction.startswith("Download"):
                # remote -> lokal (alles remote kopieren wie vorher)
                report = _sync_remote_to_local_pg(local_db, remote_dsn)
                QMessageBox.information(self, "Sync abgeschlossen", f"Download abgeschlossen.\nReport: {report}")
            else:
                # lokal -> remote: ermittele alle lokalen Tabellen und synchronisiere alles
                tables_to_sync = _all_sqlite_tables(local_db)
                # ensure remote has required tables (create from local schema) for those tables
                create_report = create_remote_schema_from_local(remote_dsn, tables=tables_to_sync, local_db=local_db)
                if create_report.get("created"):
                    QMessageBox.information(self, "Remote Schema erstellt", f"Tabellen erstellt: {create_report['created']}")
                if create_report.get("errors"):
                    QMessageBox.warning(self, "Fehler beim Erstellen Schema", f"Fehler: {create_report['errors']}")
                # then copy all table data (will use the same tables list)
                report = _sync_local_to_remote_pg(local_db, remote_dsn, tables=tables_to_sync)
                QMessageBox.information(self, "Sync abgeschlossen", f"Upload abgeschlossen.\nReport: {report}")

            self.accept()
        except Exception as e:
            import traceback
            tb = traceback.format_exc()
            QMessageBox.critical(self, "Fehler bei DB-Sync", f"{str(e)}\n\n{tb}")
