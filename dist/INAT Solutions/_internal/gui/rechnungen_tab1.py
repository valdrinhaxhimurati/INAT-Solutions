...
class RechnungenTab(QWidget):
    def __init__(self):
        ...
        self.btn_neu = QPushButton("Neue Rechnung")
        self.btn_bearbeiten = QPushButton("Rechnung bearbeiten")
        self.btn_loeschen = QPushButton("Rechnung löschen")
        self.btn_set_status = QPushButton("Status ändern")  # Auswahl: offen/bezahlt/überfällig
        self.btn_exportieren = QPushButton("Export PDF")
        self.btn_vorschau = QPushButton("Vorschau PDF")
        self.btn_layout_bearbeiten = QPushButton("Rechnungslayout bearbeiten")

        for btn in [
            self.btn_neu, self.btn_bearbeiten, self.btn_loeschen,
            self.btn_set_status,
            self.btn_exportieren, self.btn_vorschau, self.btn_layout_bearbeiten
        ]:
        ...
        self.btn_set_status.clicked.connect(self._status_aendern)  # NEU
    ...
    def aktualisiere_kunden_liste(self):
        """
        Wird aufgerufen, wenn im KundenTab ein Kunde geändert wurde.
        Lädt die Kundendaten neu und aktualisiert die Rechnungen.
        """
        self.kunden_liste = self._lade_kundennamen()
        self.kunden_adressen = self._lade_kunden_adressen()
        self.kunden_firmen = self._lade_kunden_firmen()
...
# ---------------- DB / Settings ----------------
    ...
    def initialisiere_datenbank(self):
        ...
        for spalte in ["abschluss", "uid", "firma"]:
            try:
                cursor.execute(f"ALTER TABLE rechnungen ADD COLUMN {spalte} TEXT")
    ...
    def lade_rechnungen(self):
        ...
        cursor.execute("""
            SELECT id, rechnung_nr, kunde, firma, adresse, datum, mwst, zahlungskonditionen, positionen, uid, abschluss
        ...
        for i, (id_, nr, kunde, firma, adresse, datum, mwst, zahlungskonditionen, positionen_json, uid, abschluss) in enumerate(daten):
        ...
        if rechnung_id is not None:
            ...
            cursor.execute("""
                INSERT INTO rechnungen (
                    ...
                    mwst, zahlungskonditionen, positionen, uid, abschluss
